...
enum { LEA ,IMM ,JMP ,CALL,JZ  ,JNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PUSH,
       ...
       OPEN,READ,CLOS,PRTF,MALC,MSET,MCMP,DERP,URPP,NERP,EXIT };
            ...
            if (assembly) {
                ...
                while (old_text < text) {
                    printf("%8.4s", & "LEA ,IMM ,JMP ,CALL,JZ  ,JNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PUSH,"
                                      "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                                      "OPEN,READ,CLOS,PRTF,MALC,MSET,MCMP,DERP,URPP,NERP,EXIT"[*++old_text * 5]);
	...
	GetRdtscTime(&ts1);
        ...
        if (debug) {
            printf("%d> %.4s", cycle,
                   ...
                   "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                   "OPEN,READ,CLOS,PRTF,MALC,MSET,MCMP,DERP,URPP,EXIT"[op * 5]);
            if (op <= ADJ)
                printf(" %d\n", *pc);
            else
                printf("\n");
        ...
        else if (op == DERP) { derp((char *)sp[0]); }
        else if (op == URPP) { urpp((char *)sp[1], (char *)sp[0]); }
        else if (op == NERP) { nerp(sp[1], sp[0]); }
        else {
...
int main(int argc, char **argv)
{
    ...
    src = "char else enum if int return sizeof while "
          "open read close printf malloc memset memcmp derp urpp nerp exit void main";
